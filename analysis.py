...
def likelihood1(P,DATA):
    y_fit=hypo1(DATA[0],P[0])
    return sum(stats.norm.pdf(*args) for args in zip(DATA[1],y_fit,DATA[2]))
...
def WAIC(z1,z2,z3):
    ...
    S=4000  # number of samples to take from posterior
    
    for s in range(S1-S,S1):
        P=z1.samples[s]
        l1=l1+likelihood1(P,data)
        ln1.append(log_likelihood1(P,data))
        
    lppd = np.log(l1/S) 
    pwaic= np.sum( np.square(ln1 - np.mean(ln1))  ) /(S-1.0) 
    waic1=-2.0*(lppd-pwaic)
    print(waic1)
    
    for s in range(S2-S,S2):
        P=z2.samples[s]
        l2=l2+likelihood2(P,data)
        ln2.append(log_likelihood2(P,data))
        
    lppd = np.log(l2/S) 
    pwaic= np.sum( np.square(ln2 - np.mean(ln2))  ) /(S-1.0) 
    waic2=-2.0*(lppd-pwaic)
    print(waic2)
    
    for s in range(S3-S,S3):
        P=z3.samples[s]
        l3=l3+likelihood3(P,data)
        ln3.append(log_likelihood3(P,data))
        
    lppd = np.log(l3/S) 
    pwaic= np.sum( np.square(ln3 - np.mean(ln3)) ) /(S-1.0) 
